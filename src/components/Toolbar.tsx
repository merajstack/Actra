        ...
        <div className="relative">
          ...
          {showMenu && (
            <div className="absolute top-10 right-0 w-60 bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 rounded-xl shadow-2xl z-50 py-1.5 text-xs text-zinc-700 dark:text-zinc-200">
              ...
              {userProfile && (
                <div className="px-3 py-2.5 bg-zinc-50/50 dark:bg-zinc-800/30 border-b border-zinc-100 dark:border-zinc-800 flex items-center justify-between gap-2">
                  <div className="flex items-center gap-2 overflow-hidden">
                    {userProfile.avatarUrl ? (
                      <img src={userProfile.avatarUrl} alt="Profile" className="w-7 h-7 rounded-full border border-zinc-200 dark:border-zinc-700 object-cover" />
                    ) : (
                      <div className="w-7 h-7 rounded-full bg-orange-100 dark:bg-orange-950/50 flex items-center justify-center text-orange-600 dark:text-orange-400 font-semibold text-xs shrink-0 font-sans">
                        {userProfile.displayName?.charAt(0).toUpperCase()}
                      </div>
                    )}
                    <div className="flex flex-col overflow-hidden">
                      <span className="font-semibold text-zinc-800 dark:text-zinc-200 truncate text-[11px]" title={userProfile.displayName}>
                        {userProfile.displayName}
                      </span>
                      <span className="text-[9px] text-zinc-400 dark:text-zinc-500">Signed In</span>
                    </div>
                  </div>
                  {onLogout && (
                    <button
                      ...
                      className="flex items-center justify-center p-1.5 rounded-lg text-zinc-400 hover:text-red-500 hover:bg-red-50 dark:hover:bg-red-950/20 transition-all cursor-pointer border border-transparent hover:border-red-200 dark:hover:border-red-900/30"
